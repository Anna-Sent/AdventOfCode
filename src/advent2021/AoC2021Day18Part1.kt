...
private sealed class Leaf2021181 {

    data class Regular(val value: Int) : Leaf2021181() {
        override fun toString(): String {
            return value.toString()
        }
    }

    data class Complex(val first: Leaf2021181, val second: Leaf2021181) : Leaf2021181() {
        override fun toString(): String {
            return "[$first,$second]"
        }
    }
...
private fun test(input: String, print: Boolean): Int {
    return input.split("\n").map { it.parse() }.reduce { acc, leaf -> acc.add(leaf, print) }.magnitude()
}

private fun Leaf2021181.magnitude(): Int = when (this) {
    is Leaf2021181.Complex -> 3 * first.magnitude() + 2 * second.magnitude()
    is Leaf2021181.Regular -> value
}

private fun Leaf2021181.add(other: Leaf2021181, print: Boolean): Leaf2021181 {
    var result: Leaf2021181 = Leaf2021181.Complex(this, other)
...
private fun Leaf2021181.explodeLoop(print: Boolean): Leaf2021181 {
    var result = this
    var prev: Leaf2021181? = null
    while (prev != result) {
        prev = result
...
private fun Leaf2021181.explode(print: Boolean): Leaf2021181 {
...
private fun Leaf2021181.split(print: Boolean): Leaf2021181 {
    val list = toList().toMutableList()
...
private fun Leaf2021181.toList(): List<Int> {
    val s = toString()
...
private fun List<Int>.fromList(): Leaf2021181 {
    var s = ""
    for (j in indices) {
        val item = this[j]
        when {
...
private fun String.parse(): Leaf2021181 {
    when {
        this[0] == '[' -> {
            var s1 = ""
            var i = 1
            ...
            return Leaf2021181.Complex(s1.parse(), s2.parse())
        }
        this[0] in '0'..'9' -> {
            var j = 0
            var num = ""
            ...
            return Leaf2021181.Regular(num.toInt())
        }
