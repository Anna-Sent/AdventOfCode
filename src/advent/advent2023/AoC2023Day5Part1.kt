...
fun main() {
    ...
    check(379811651, result)
}

private fun test(input: String): Long {
    val seeds = mutableListOf<Long>()
    val seedToSoilMap = mutableListOf<Node1>()
    val soilToFertilizerMap = mutableListOf<Node1>()
    val fertilizerToWaterMap = mutableListOf<Node1>()
    val waterToLightMap = mutableListOf<Node1>()
    val lightToTemperatureMap = mutableListOf<Node1>()
    val temperatureToHumidityMap = mutableListOf<Node1>()
    val humidityToLocationMap = mutableListOf<Node1>()
    ...
    while (i < lines.size) {
        ...
        if (line.startsWith("seeds: ")) {
            ...
            while (lines[i].isNotEmpty()) {
                ...
                seedToSoilMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                soilToFertilizerMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                fertilizerToWaterMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                waterToLightMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                lightToTemperatureMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                temperatureToHumidityMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (i < lines.size && lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                humidityToLocationMap += Node1(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
        }
        ++i
