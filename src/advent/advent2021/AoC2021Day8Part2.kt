...
fun main() {
    ...
    check(1007675, result)
}

private val unique = setOf(1, 4, 7, 8)
...
private val reversedMap = map.entries.associate { (k, v) -> v to k }
...
private fun solve(input: String): Int {
    val (patterns, output) = input.split(" | ").map { it.split(" ").toList() }

    fun match(n: Int, s: String) = map[n]!!.size == s.length
    fun String.matchUnique(): Int {
        for (u in unique) {
            if (match(u, this)) {
                return u
            }
    ...
    val guessed = mutableMapOf<Set<Char>, Set<Char>>()

    for (item in patterns) {
        val set = item.toSet()
        if (guessed.containsKey(set)) {
            continue
        }
        val u = item.matchUnique()
        if (u != -1) {
            guessed[map[u]!!] = set
        }
    }
    ...
    val keyVariant = "abcdefg".toList()
    val variants = keyVariant.permutations()

    var validMapping: MutableMap<Char, Char>? = null

    for (variant in variants) {
        val mapping = mutableMapOf<Char, Char>()
        val reversedMapping = mutableMapOf<Char, Char>()
        for (i in keyVariant.indices) {
            mapping[keyVariant[i]] = variant[i]
            reversedMapping[variant[i]] = keyVariant[i]
        }

        var invalid = false

        for ((from, to) in guessed) {
            val check = from.map { mapping[it] }.toSet()
            if (check != to) {
                invalid = true
                break
            }
        ...
        for (item in output) {
            val set = item.toSet().map { reversedMapping[it]!! }.toSet()
            if (!reversedMap.containsKey(set)) {
                invalid = true
                break
            }
        }
        ...
        validMapping = reversedMapping
        break
    }

    val mapping = validMapping!!

    var number = ""
    for (item in output) {
        val set = item.toSet().map { mapping[it]!! }.toSet()
        val digit = reversedMap[set]!!
        number += digit
    }
    return number.toInt()
}
