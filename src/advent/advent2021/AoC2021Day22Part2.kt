...
fun main() {
    ...
    check(1201259791805392, result)
}

private val pattern = "(on|off) x=(-?\\d+)..(-?\\d+),y=(-?\\d+)..(-?\\d+),z=(-?\\d+)..(-?\\d+)".toPattern()

private data class Cube(val from: Point3D, val to: Point3D) {

    val rx get() = from.x..to.x
    val ry get() = from.y..to.y
    val rz get() = from.z..to.z

    constructor(rx: IntRange, ry: IntRange, rz: IntRange) : this(
        Point3D(rx.first, ry.first, rz.first), Point3D(rx.last, ry.last, rz.last)
    )
}

private operator fun Cube.minus(other: Cube): List<Cube> {
    if (this in other) return emptyList()
    val dx = rx.split(other.rx)
    val dy = ry.split(other.ry)
    val dz = rz.split(other.rz)
    val result = mutableListOf<Cube>()
    for (xx in dx) {
        for (yy in dy) {
            for (zz in dz) {
                val cube = Cube(xx, yy, zz)
                if (cube !in other) {
                    result += cube
                }
            }
...
private fun IntRange.split(other: IntRange): List<IntRange> {
    if (other == this) return listOf(this)
    if (this in other) return listOf(this)
    if (other in this) {
        if (first == other.first) {
            return listOf(first..other.last, other.last + 1..last)
        }
        if (last == other.last) {
            return listOf(first..other.first - 1, other.first..last)
        }
        return listOf(first..other.first - 1, other.first..other.last, other.last + 1..last)
    }
    if (first in other) {
        return listOf(first..other.last, other.last + 1..last)
    }
...
private operator fun Cube.contains(other: Cube) = other.rx in rx && other.ry in ry && other.rz in rz

private operator fun IntRange.contains(other: IntRange) = first <= other.first && other.last <= last

private fun Cube.vol() = (to.x.toLong() - from.x + 1) * (to.y - from.y + 1) * (to.z - from.z + 1)
...
private fun test(input: String): Long {
    ...
    for (command in commands) {
        ...
        if (matcher.matches()) {
            val isOn = matcher.group(1) == "on"

            val startX = matcher.group(2).toInt()
            val endX = matcher.group(3).toInt()
            val startY = matcher.group(4).toInt()
            val endY = matcher.group(5).toInt()
            val startZ = matcher.group(6).toInt()
            val endZ = matcher.group(7).toInt()

            val newCube = Cube(Point3D(startX, startY, startZ), Point3D(endX, endY, endZ))

            val newCubes = mutableListOf<Cube>()
            for (cube in cubes) newCubes += cube - newCube
            if (isOn) newCubes += newCube

            cubes = newCubes
        } else throw IllegalArgumentException(command)
    }

    return cubes.map { it.vol() }.sum()
}
