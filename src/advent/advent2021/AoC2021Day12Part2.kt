...
private fun test(input: String): Int {
    ...
    find(mutableListOf("start"), false, map, paths)
    return paths.size
}
...
private fun find(current: MutableList<String>,
    ...
    for (p in map[current.last()]!!) {
        if (p == "start") continue
        if (p.isSmall() && p in current && usedTwice) {
            continue
        }
        val newCurrent = mutableListOf<String>()
        newCurrent += current
        newCurrent += p
        val newUsedTwice = usedTwice || p.isSmall() && p in current
        if (p == "end") {
            paths += newCurrent
        } else {
            find(newCurrent, newUsedTwice, map, paths)
        }
    }
}

private fun String.isSmall() = this[0] in 'a'..'z' && this != "start" && this != "end"
