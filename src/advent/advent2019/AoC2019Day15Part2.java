...
public class AoC2019Day15Part2 {
    ...
    public static void main(String[] args) {
        ...
        assert result == 302 : "unexpected result is " + result;
    ...
    private static int test(String s) {
        ...
        Point o = bfs(map, initialState);
        System.out.println(o);

        int count = 0;
        while (!isFilled(map)) {
            Set<Point> oxygenPoints = oxygenPoints(map);
            fillAdjacent(map, oxygenPoints);
            ++count;
        }
    ...
    private static boolean isFilled(Map<Point, Character> map) {
    ...
    private static Set<Point> oxygenPoints(Map<Point, Character> map) {
    ...
    private static void fillAdjacent(Map<Point, Character> map,
                                     Set<Point> oxygenPoints) {
        for (Point point : oxygenPoints) {
            fillAdjacent(map, new Point(point.x - 1, point.y));
            fillAdjacent(map, new Point(point.x + 1, point.y));
            fillAdjacent(map, new Point(point.x, point.y - 1));
            fillAdjacent(map, new Point(point.x, point.y + 1));
        }
    }
    ...
    private static void fillAdjacent(Map<Point, Character> map,
                                     Point oxygenPoint) {
        Character value = map.get(oxygenPoint);
        if (value == '.') {
            map.put(oxygenPoint, 'O');
        }
    }

    private static Point bfs(Map<Point, Character> map, State initialState) {
        Point o = null;
        ...
        while (opened.size() > 0) {
            ...
            for (State current : opened) {
                if (o == null && map.getOrDefault(current.point, '.') == 'O') {
                    o = current.point;
                }
