...
public class AoC2019Day18Part2_deadlock {
    ...
    private static int test(int number, String s) {
        ...
        while (!solved(solutions)) {
            for (int i = 0; i < 4; ++i) {
                ...
                // System.out.println(number + " input " + i + " collected: " + collected);
                int solution = bfs(map, allKeysCollected, new State(initialPoints.get(i), collected));
    ...
    private static boolean solved(int[] solutions) {
        for (int solution : solutions) {
            if (solution <= 0) {
                return false;
            }
