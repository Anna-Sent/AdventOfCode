...
private fun test(input: String): Int {
        ...
        when (move) {
            '>' -> {
                ...
                if (next !in walls) {
                    ...
                    while (nextBox in boxes) {
                        movingBoxes += nextBox
                        nextBox = Point(nextBox.x, nextBox.y + 1)
                    }
                    if (nextBox !in walls) {
                        robot = next
                        boxes -= movingBoxes
                        for (box in movingBoxes) {
                            boxes += Point(box.x, box.y + 1)
                        }
                    }
                }
            }
            ...
            '^' -> {
                ...
                if (next !in walls) {
                    ...
                    while (nextBox in boxes) {
                        movingBoxes += nextBox
                        nextBox = Point(nextBox.x - 1, nextBox.y)
                    }
                    if (nextBox !in walls) {
                        robot = next
                        boxes -= movingBoxes
                        for (box in movingBoxes) {
                            boxes += Point(box.x - 1, box.y)
                        }
                    }
                }
            }
    ...
    return boxes.map { it.x * 100 + it.y }.sum()
}
