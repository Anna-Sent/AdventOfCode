...
private fun isSafe(numbers: List<Int>): Boolean {
    ...
    val sign = abs(numbers[0] - numbers[1]) / (numbers[0] - numbers[1])
    for (i in 0..<numbers.lastIndex) {
        if (!(numbers[i] - numbers[i + 1] in sign * 1..sign * 3 || numbers[i] - numbers[i + 1] in sign * 3..sign * 1)) {
            return false
        }
    }
    return true
