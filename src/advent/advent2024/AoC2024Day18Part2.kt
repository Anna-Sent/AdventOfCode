...
private fun test(input: String, size: Int): String {
    ...
    fun bfs(startPoint: Point, bytes: List<Point>): Int {
        var opened = mutableSetOf(startPoint)
        val closed = mutableSetOf<Point>()
        var steps = 0
        while (opened.isNotEmpty()) {
            val achievable = mutableSetOf<Point>()
            for (current in opened) {
                if (current == Point(size, size)) {
                    return steps
                }
                val set = current.generateNext(bytes)
                for (next in set) {
                    if (next !in closed) {
                        achievable += next
                    }
    ...
    for (i in 1..bytes.size) {
        val steps = bfs(Point(0, 0), bytes.subList(0, i))
        if (steps == -1) {
            val byte = bytes[i - 1]
