...
private fun test(input: String): Int {
    val (rulesStr, listsStr) = input.split("\n\n")
    val pairs = rulesStr.split("\n").map {
        val (first, second) = it.split("|").map(String::toInt)
        first to second
    }
    val fromToMap = mutableMapOf<Int, MutableSet<Int>>()
    for (pair in pairs) {
        fromToMap.putIfAbsent(pair.first, mutableSetOf())
        fromToMap[pair.first]!! += pair.second
    }
    val lists = listsStr.split("\n").map {
        it.split(",").map(String::toInt)
    }

    fun isOrderCorrect(v1: Int, v2: Int): Boolean {
        val map = fromToMap[v1]
        return map != null && v2 in map
    }

    fun isOrderCorrect(list: List<Int>): Boolean {
        for (i in 1..list.lastIndex) {
            if (!isOrderCorrect(list[i - 1], list[i])) {
                return false
            }
        }
        return true
    ...
    var s = 0
    for (list in lists) {
        if (!isOrderCorrect(list)) {
            val sortedList = list.sortedWith(object : Comparator<Int> {
                override fun compare(v1: Int, v2: Int): Int {
                    if (v1 == v2) {
                        return 0
                    }
                    if (isOrderCorrect(v1, v2)) {
                        return -1
                    }
                    return 1
                }
            })
            s += sortedList[sortedList.size / 2]
        }
    }
