...
private fun test(input: String): Int {
    var prevState: List<CharArray>? = null
    var currState: List<CharArray>? = input.split("\n").map { CharArray(it.length) { i -> it[i] } }

    while (!(prevState eq currState)) {
        val tmp = currState
        currState = generateNext(currState!!)
        prevState = tmp
    }
    return currState!!.map { it.count { ch -> ch == '#' } }.sum()
}

private infix fun List<CharArray>?.eq(other: List<CharArray>?): Boolean {
    if (this == null) return other == null
    if (other == null) return false
    if (size != other.size) return false
    for (i in other.indices) {
        if (!(get(i) contentEquals other[i])) {
            return false
        }
    }
    return true
...
private fun generateNext(state: List<CharArray>): List<CharArray> {
    val nextState: List<CharArray> = mutableListOf<CharArray>().apply {
        state.forEach { add(it.copyOf()) }
    }
    for (i in nextState.indices) {
        val row = nextState[i]
        for (j in row.indices) {
            val seat = seat(state, i, j)
            val adjacent = adjacent(state, i, j)
            val occupiedCount = adjacent.count { it == '#' }
            if (seat == 'L') {
                if (occupiedCount == 0) {
                    row[j] = '#'
                }
...
private fun seat(state: List<CharArray>, i: Int, j: Int): Char {
    return state.getOrElse(i) { empty }.getOrElse(j) { '$' }
}

private fun seat(state: List<CharArray>, i: Int, j: Int, di: Int, dj: Int): Char {
    var ch = ' '
    var ci = i
    var cj = j
    do {
...
private fun adjacent(state: List<CharArray>, i: Int, j: Int): List<Char> {
    val list = mutableListOf<Char>()
    list.add(seat(state, i, j, 0, 1))
    list.add(seat(state, i, j, 0, -1))
    list.add(seat(state, i, j, 1, 0))
    list.add(seat(state, i, j, -1, 0))
    list.add(seat(state, i, j, 1, 1))
    list.add(seat(state, i, j, -1, -1))
    list.add(seat(state, i, j, 1, -1))
    list.add(seat(state, i, j, -1, 1))
    return list
