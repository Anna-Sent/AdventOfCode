...
private val pattern = Pattern.compile("(\\d+) (\\w+ \\w+) bags?")

private fun test(input: String): Int {
    val rules = input.split("\n")
    val bags = mutableMapOf<String, List<Pair<String, Int>>>()
    for (rule in rules) {
        var tokens = rule.split(" bags contain ")
        val containerBag = tokens[0]
        val list = tokens[1].substring(0, tokens[1].length - 1)
        tokens = list.split(", ")
        val childBags = mutableListOf<Pair<String, Int>>()
        for (token in tokens) {
            if (token == "no other bags") {
                ...
                val childBag = matcher.group(2)
                val count = matcher.group(1).toInt()
                childBags += childBag to count
            }
        }
        bags[containerBag] = childBags
    }
    return countOfChildren(bags, SHINY_GOLD)
}

private const val SHINY_GOLD = "shiny gold"

private fun countOfChildren(bags: Map<String, List<Pair<String, Int>>>, bag: String): Int {
    val childBags = bags[bag]!!
    var sum = 0
    for ((childBag, count) in childBags) {
        val childCount = countOfChildren(bags, childBag)
        sum += count * childCount + count
    }
    return sum
}
