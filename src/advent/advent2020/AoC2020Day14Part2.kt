...
private val maskPattern = Pattern.compile("mask = (.+)")
private val memPattern = Pattern.compile("mem\\[(\\d+)\\] = (\\d+)")
...
private fun test(input: String): Long {
    ...
    for (command in commands) {
        var matcher = maskPattern.matcher(command)
        if (matcher.matches()) {
            val mask = matcher.group(1)
            currentMask = mask
        } else {
            matcher = memPattern.matcher(command)
            if (matcher.matches()) {
                val address = matcher.group(1).toLong()
                val value = matcher.group(2).toLong()
                val valueBin = toBinaryString(address)
                val valueMasked = valueBin.applyMask(currentMask!!)
                println("address:\t$valueBin")
                println("mask:\t\t$currentMask")
                println("result:\t\t${valueMasked}")
                println()
                for (floatedAddress in floatedAddresses(valueMasked)) {
                    memory[fromBinaryString(floatedAddress)] = value
                }
            } else {
                throw IllegalArgumentException("not matches $command")
            }
        }
    }
...
private fun toBinaryString(value: Long): String {
    val result = java.lang.Long.toBinaryString(value)
    return if (result.length > 36) {
        println("got it!")
        result.substring(result.length - 36, result.length)
    } else {
        String(CharArray(36 - result.length) { '0' }) + result
    }
}

private fun fromBinaryString(value: String): Long {
    return java.lang.Long.parseLong(value, 2)
}

private fun String.applyMask(mask: String): String {
    val sb = StringBuilder()
    for (i in 0..35) {
        when {
            mask[i] == '0' -> sb.append(this[i])
            mask[i] == '1' -> sb.append('1')
            else -> sb.append('X')
        }
    }
    return sb.toString()
...
private fun floatedAddresses(mask: String): List<String> {
    val result = mutableListOf<String>()
    val xCount = mask.count { it == 'X' }
    val max = 2.0.pow(xCount.toDouble()).toLong()
    for (i in 0..max) {
        var binary = java.lang.Long.toBinaryString(i)
        binary = if (binary.length < xCount) {
            String(CharArray(xCount - binary.length) { '0' }) + binary
        } else {
            binary
        }
        var binaryCount = 0
