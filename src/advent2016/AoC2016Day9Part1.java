...
public class AoC2016Day9Part1 {
    ...
    static int solution1(String s) {
        ...
        for (int i = 0; i < s.length(); ) {
            ...
            if (ch == '(') {
                ...
                if (ch == 'x') {
                    int length = Integer.parseInt(tmp.toString());
    ...
    static long bf(String s) {
        if (s.length() == 0) {
            return 0;
        }

        Pattern p = Pattern.compile("([A-Z]+)([A-Zx0-9\\)\\(]*)");
        Matcher m = p.matcher(s);
        if (m.matches()) {
            String head = m.group(1);
            String tail = m.group(2);
            return head.length() + bf(tail);
        } else {
            p = Pattern.compile("\\((\\d+)x(\\d+)\\)([A-Zx0-9\\(\\)]+)");
            m = p.matcher(s);
            if (m.matches()) {
                int length = Integer.parseInt(m.group(1));
                int times = Integer.parseInt(m.group(2));
                String tail = m.group(3);
                String middle = tail.substring(0, length);
                tail = tail.substring(length);
                // return times * bf(middle) + bf(tail);
                return times * middle.length() + bf(tail);
            } else {
