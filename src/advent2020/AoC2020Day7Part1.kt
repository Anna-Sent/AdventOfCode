...
private fun test(input: String): Int {
    ...
    return bags.keys.count { canContainShinyGold(bags, it) } - 1
}

private const val SHINY_GOLD = "shiny gold"

private fun canContainShinyGold(bags: Map<String, List<Pair<String, Int>>>, bag: String): Boolean {
    if (bag == SHINY_GOLD) {
        return true
    } else {
        val childBags = bags[bag]
        for ((childBag, _) in childBags!!) {
            if (canContainShinyGold(bags, childBag)) {
                return true
            }
        }
        return false
