...
private fun test(input: String): Int {
    ...
    return passports.count { isValid(it) && isValidFields(it) }
}

private val required = setOf("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid")

private fun isValid(passport: String): Boolean {
    return required.all { passport.contains(it) }
}

private fun isValidFields(passport: String): Boolean {
    val entries = passport.split(Pattern.compile("\\s+"))
    return entries.all { isValidEntry(it) }
}
...
private fun isValidEntry(entry: String): Boolean {
    ...
    when (key) {
        ...
        "hgt" -> {
            val height = value.substring(0, value.length - 2).toIntOrNull()
            return when {
                value.endsWith("cm") -> height in 150..193
                value.endsWith("in") -> height in 59..76
                else -> false
            }
        }
        "hcl" -> {
            return if (value.startsWith("#")) {
                val color = value.substring(1, value.length - 1)
                color.all { it in '0'..'9' || it in 'a'..'f' }
            } else {
                false
            }
        }
        ...
        "pid" -> {
            return value.length == 9 && value.all { it in '0'..'9' }
        }
        "cid" -> {
            return true
        }
