...
private fun test(input: String): Int {
    ...
    for (entry in entries) {
        ...
        assert(matcher.matches()) { "$entry not matches" }
        val low = matcher.group(1).toInt()
        val high = matcher.group(2).toInt()
        assert(matcher.group(3).length == 1) { "$entry not matches letter" }
        val letter = matcher.group(3)[0]
        val word = matcher.group(4)

        val oneOf = word[low - 1] == letter || word[high - 1] == letter
        val both = word[low - 1] == letter && word[high - 1] == letter
        if (oneOf && !both) {
            ++count
        }
    }
