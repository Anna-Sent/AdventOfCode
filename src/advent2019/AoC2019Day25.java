...
public class AoC2019Day25 {
    ...
    private static void test(String s) {
        ...
        String output = getOutput(commands);
        State initialState = new State(commands, Collections.emptySet());
        bfs(fillState(initialState, output));
    }
    ...
    private static void bfs(State initialState) {
        ...
        while (opened.size() > 0) {
            ...
            for (State current : opened) {
                if (current.room.equals("Pressure-Sensitive Floor")) {
                    System.out.println(current.output);
                    return;
                }
                Set<State> set = generateNext(current);
                for (State next : set) {
    ...
    private static Set<State> generateNext(State current) {
        Set<State> set = new HashSet<>();

        Set<Set<String>> subsets = new HashSet<>();
        subsets.add(Collections.emptySet());
        if (current.itemInRoom != null && !forbidden.contains(current.itemInRoom)) {
            subsets.add(Collections.singleton(current.itemInRoom));
        }
        for (String door : current.doorsInRoom) {
            for (Set<String> subset : subsets) {
                List<String> newCommands = new ArrayList<>(current.commands);
                Set<String> newItems = new HashSet<>(current.items);
                for (String item : subset) {
                    newCommands.add("take " + item);
                    newItems.add(item);
                }
                newCommands.add(door);
                String newOutput = getOutput(newCommands);
                State newState = new State(newCommands, newItems);
                set.add(fillState(newState, newOutput));
            }
        }
    ...
    private static State fillState(State state, String output) {
        ...
        for (int i = tokens.length - 1; i >= 0; --i) {
            ...
            if (token.startsWith("==")) {
                room = token.substring(3, token.length() - 3);
                break;
            } else if (token.equals("Doors here lead:")) {
                int j = i + 1;
                while (tokens[j].startsWith("-")) {
                    doors.add(tokens[j].substring(2));
                    ++j;
                }
            } else if (token.equals("Items here:")) {
                int j = i + 1;
                while (tokens[j].startsWith("-")) {
                    items.add(tokens[j].substring(2));
                    ++j;
                }
            }
        }
        assert room != null : "invalid input";
        assert items.size() == 1 || items.isEmpty() : "invalid input";
        state.room = room;
        state.itemInRoom = items.isEmpty() ? null : items.get(0);
        state.doorsInRoom = doors;
        state.output = output;
        return state;
    }

    private static String getOutput(List<String> commands) {
        List<Long> inputs = new ArrayList<>();
        for (String command : commands) {
            for (int i = 0; i < command.length(); ++i) {
                inputs.add((long) command.charAt(i));
            }
            inputs.add((long) '\n');
        }
        ...
        return program.output.toString();
    }
    ...
    private static class State {
        ...
        String room;
        String itemInRoom;
        Set<String> doorsInRoom;
        String output;

        State(List<String> commands, Set<String> items) {
        ...
        public boolean equals(Object o) {
            ...
            State state = (State) o;
            return Objects.equals(room, state.room)
                    && Objects.equals(items, state.items);
        }

        @Override
        public int hashCode() {
            return Objects.hash(room, items);
        }
    }
        ...
        for (; !state.completed; ) {
            ...
            switch (DE) {
                ...
                case 4: {
                    ...
                    state.output.append((char) result);
