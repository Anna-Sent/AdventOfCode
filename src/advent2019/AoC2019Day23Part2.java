...
public class AoC2019Day23Part2 {
    ...
    public static void main(String[] args) {
        ...
        result = test(input);
        assert result == 17265 : "unexpected result is " + result;
    ...
    private static long test(String s) {
        ...
        Long lastX = null, lastY = null, lastSentY = null;
        Set<Integer> idles = new HashSet<>();
        while (true) {
            idles.clear();
            for (int i = 0; i < 50; ++i) {
                ProgramState program = programs.get(i);
                boolean noInput = program.inputs.size() == program.inputCounter;
                if (noInput) {
                    programs.get(i).inputs.add(-1L);
                }
                List<Long> outputs = program.outputs;
                run(program);
                boolean noOutput = outputs.isEmpty();
                if (noInput && noOutput) {
                    idles.add(i);
                }
            ...
            if (idles.size() == 50) {
                assert lastX != null : "no NAT";
                programs.get(0).inputs.add(lastX);
                programs.get(0).inputs.add(lastY);
                if (lastSentY != null && lastSentY.equals(lastY)) {
                    return lastY;
                }
                lastSentY = lastY;
            }
