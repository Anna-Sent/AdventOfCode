...
public class AoC2019Day3Part2 {
    ...
    public static void main(String[] args) {
        ...
        assert result == 63526 : "unexpected result is " + result;
        System.out.println(result);
    }

    private static int test(String p1, String p2) {
        Map<Point, Integer> m1 = points(p1);
        Map<Point, Integer> m2 = points(p2);

        Set<Point> s1 = m1.keySet();
        Set<Point> s2 = m2.keySet();
        s1.retainAll(s2);

        Point closest = null;
        int closestDistance = -1;
        for (Point p : s1) {
            int distance = m1.get(p) + m2.get(p);
    ...
    private static Map<Point, Integer> points(String p) {
