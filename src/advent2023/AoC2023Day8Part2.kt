...
fun main() {
    ...
    check(17972669116327, result)
}

private fun test(input: String): Long {
    val (instructions, lines) = input.split("\n\n")
    val map = mutableMapOf<String, Pair<String, String>>()
    val allNodes = mutableSetOf<String>()
    for (line in lines.split("\n")) {
        val (from, token) = line.split(" = ")
        val (left, right) = token.removePrefix("(").removeSuffix(")").split(", ")
    ...
    currentNodes += allNodes.filter { it.endsWith('A') }
    var count = 0
    var index = 0
    val endNodes = mutableMapOf<String, Int>()
    endNodes.putAll(allNodes.filter { it.endsWith('Z') }.map { it to 0 })
    while (true) {
        val instruction = instructions[index]
        val nextNodes = mutableListOf<String>()
        for (node in currentNodes) {
            val nextNode = if (instruction == 'L') map[node]!!.first else map[node]!!.second
            nextNodes += nextNode
        }
        ...
        val endingNodes = nextNodes.filter { it.endsWith('Z') }
        for (node in endingNodes) {
            if (endNodes[node] == 0) endNodes[node] = count
        }
        if (endNodes.all { it.value > 0 }) {
            break
        }
    }
    return lcm(endNodes.values.map { it.toLong() }.toList())
}
