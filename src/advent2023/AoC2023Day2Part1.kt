...
private fun test(input: String): Int {
    ...
    for (s in input.split("\n")) {
        ...
        val (t1, t2) = s.split(": ")
        val gameId = t1.removePrefix("Game ").toInt()
        val sets = t2.split("; ")
        ...
        for (set in sets) {
            ...
            for (ball in balls) {
                ...
                if (ball.endsWith(RED)) {
                    red = ball.removeSuffix(RED).trimEnd().toInt()
                } else if (ball.endsWith(GREEN)) {
                    green = ball.removeSuffix(GREEN).trimEnd().toInt()
                } else if (ball.endsWith(BLUE)) {
                    blue = ball.removeSuffix(BLUE).trimEnd().toInt()
                }

                isGamePossible = isGamePossible && red <= 12 && green <= 13 && blue <= 14
            }
        }
