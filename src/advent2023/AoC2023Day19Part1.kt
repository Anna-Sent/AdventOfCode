...
private fun test(input: String): Int {
    val (token1, token2) = input.split("\n\n")

    val rules = mutableMapOf<String, Rule191>()
    val variables = mutableListOf<Map<String, Int>>()

    for (line in token1.split("\n")) {
        val (ruleName, token) = line.split("{")
        val tokens = token.removePrefix("}").split(",")
        val conditions = mutableListOf<Condition191>()
        for (conditionToken in tokens.subList(0, tokens.lastIndex)) {
            if (conditionToken.contains(">")) {
                ...
                val condition = Condition191(operandName, Operation191.G, numberString.toInt(), result)
                conditions += condition
            } else if (conditionToken.contains("<")) {
                val (operandName, t) = conditionToken.split("<")
                val (numberString, result) = t.split(":")
                val condition = Condition191(operandName, Operation191.L, numberString.toInt(), result)
                conditions += condition
            } else throw IllegalStateException("impossible $conditionToken")
        }
        val further = tokens[tokens.lastIndex].removeSuffix("}")
        val rule = Rule191(ruleName, conditions, further)
        rules[ruleName] = rule
    }

    for (line in token2.split("\n")) {
        val tokens = line.removePrefix("{").removeSuffix("}").split(",")
        val variablesMap = mutableMapOf<String, Int>()
        for (token in tokens) {
            val (variableName, numberString) = token.split("=")
            variablesMap[variableName] = numberString.toInt()
        }
        variables += variablesMap
    }

    var sum = 0
    for (variablesMap in variables) {
        println(variablesMap)
        var currentRule = "in"
        while (currentRule !in setOf("A", "R")) {
            println(currentRule)
            ...
            for (condition in rule.conditions) {
                when (condition.operation) {
                    Operation191.G -> {
                        if (variablesMap[condition.operandName]!! > condition.number) {
                            currentRule = condition.result
                            changed = true
                            break
                        }
                    }
            ...
            if (!changed) {
                currentRule = rule.further
            }
        }
        println(currentRule)
        if (currentRule == "A") {
            for (variable in variablesMap.keys) {
                sum += variablesMap[variable]!!
            }
        }
    }
...
private enum class Operation191 { G, L }

private data class Condition191(
    val operandName: String,
    val operation: Operation191,
    val number: Int,
    val result: String
)

private data class Rule191(
    val name: String,
    val conditions: List<Condition191>,
    val further: String
)
