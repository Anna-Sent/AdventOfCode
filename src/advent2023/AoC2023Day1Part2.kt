...
private fun test(input: String): Int {
    ...
    return input.split("\n").map {
        ...
        val index1 = it.findAnyOf(digits.keys) ?: (-1 to "")
        val index2 = it.indexOfFirst(Char::isDigit)
        val first = if (index1.first >= 0 && index1.first < index2 || index2 < 0) {
            digits[index1.second]
        } else {
            it[index2].toString()
        }

        val index3 = it.findLastAnyOf(digits.keys) ?: (-1 to "")
        val index4 = it.indexOfLast(Char::isDigit)
        val second = if (index3.first > index4) {
            digits[index3.second]
        } else {
            it[index4].toString()
        }
