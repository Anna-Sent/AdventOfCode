        ...
        for (set in sets) {
            ...
            for (ball in balls) {
                if (ball.endsWith(RED)) {
                    red = max(red, ball.removeSuffix(RED).trimEnd().toInt())
                } else if (ball.endsWith(GREEN)) {
                    green = max(green, ball.removeSuffix(GREEN).trimEnd().toInt())
                } else if (ball.endsWith(BLUE)) {
                    blue = max(blue, ball.removeSuffix(BLUE).trimEnd().toInt())
                }
            }
        }
