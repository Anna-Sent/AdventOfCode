...
private fun test(input: String): Int {
    ...
    val possibleGears = mutableMapOf<Point, MutableList<Int>>()
    for ((lineNumber, line) in lines.withIndex()) {
        ...
        while (i < line.length) {
            if (line[i].isDigit()) {
                ...
                val endExclusive = i
                val number = String(digits.toCharArray()).toInt()

                val rectangle = mutableSetOf<Point>()
                for (k in start - 1..endExclusive) rectangle += Point(k, lineNumber - 1)
                rectangle += Point(start - 1, lineNumber)
                rectangle += Point(endExclusive, lineNumber)
                for (k in start - 1..endExclusive) rectangle += Point(k, lineNumber + 1)
                ...
                for (p in rectangle) {
                    if (p.x in 0 until width && p.y in 0 until height) {
                        val ch = lines[p.y][p.x]
                        if (ch !in symbols) throw RuntimeException("ch = $ch")
                        isPartNumber = isPartNumber || ch != '.'
                        if (ch == '*') stars += p
                    }
                }
                if (isPartNumber) {
                    for (star in stars) {
                        possibleGears.putIfAbsent(star, mutableListOf())
                        possibleGears[star]!!.add(number)
                    }
                }
            } else {
                ++i
    ...
    return possibleGears.filter { (_, numbers) -> numbers.count() == 2 }.map { (_, numbers) -> numbers[0] * numbers[1] }
        .sum()
}
