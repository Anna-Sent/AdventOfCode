...
fun main() {
    ...
    check(27992443, result)
}

private fun test(input: String): Long {
    val seeds = mutableListOf<Long>()
    val seedToSoilMap = mutableListOf<Node2>()
    val soilToFertilizerMap = mutableListOf<Node2>()
    val fertilizerToWaterMap = mutableListOf<Node2>()
    val waterToLightMap = mutableListOf<Node2>()
    val lightToTemperatureMap = mutableListOf<Node2>()
    val temperatureToHumidityMap = mutableListOf<Node2>()
    val humidityToLocationMap = mutableListOf<Node2>()
    ...
    while (i < lines.size) {
        ...
        if (line.startsWith("seeds: ")) {
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                seedToSoilMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                soilToFertilizerMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                fertilizerToWaterMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                waterToLightMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                lightToTemperatureMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                temperatureToHumidityMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
            ...
            while (i < lines.size && lines[i].isNotEmpty()) {
                val (destinationRangeStart, sourceRangeStart, rangeLength) = lines[i].split(" ").map { it.toLong() }
                humidityToLocationMap += Node2(destinationRangeStart, sourceRangeStart, rangeLength)
                ++i
            }
        }
        ++i
    ...
    while (seedIndex + 1 < seeds.size) {
        println("seeds range from $seedIndex to ${seedIndex + 1}")
        for (seed in seeds[seedIndex] until seeds[seedIndex] + seeds[seedIndex + 1]) {
            var location = seed
            for (node in seedToSoilMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
            }
            for (node in soilToFertilizerMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
            }
            for (node in fertilizerToWaterMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
            }
            for (node in waterToLightMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
            }
            for (node in lightToTemperatureMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
            }
            for (node in temperatureToHumidityMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
            }
            for (node in humidityToLocationMap) {
                if (location in node.sourceRangeStart until node.sourceRangeStart + node.rangeLength) {
                    location = node.destinationRangeStart + location - node.sourceRangeStart
                    break
                }
        ...
        println("current lowest $lowestLocation")
        seedIndex += 2
    }
...
private data class Node2(
    val destinationRangeStart: Long, val sourceRangeStart: Long, val rangeLength: Long
)
