...
private fun test(input: String): Int {
    ...
    loop@ while (true) {
        val p = loop.last()
        when (lines[p.y][p.x]) {
            '|' -> loop += if (p.y + 1 in 0 until height && lines[p.y + 1][p.x] in setOf('|', 'L', 'J', 'S')
                && Point(p.x, p.y + 1) !in loop
            ) {
                Point(p.x, p.y + 1)
            } else
                if (p.y - 1 in 0 until height && lines[p.y - 1][p.x] in setOf('|', 'F', '7', 'S')
                    && Point(p.x, p.y - 1) !in loop
                ) {
                    Point(p.x, p.y - 1)
                } else break@loop

            '-' -> loop += if (p.x + 1 in 0 until width && lines[p.y][p.x + 1] in setOf('-', 'J', '7', 'S')
                && Point(p.x + 1, p.y) !in loop
            ) {
                Point(p.x + 1, p.y)
            } else
                if (p.x - 1 in 0 until width && lines[p.y][p.x - 1] in setOf('-', 'L', 'F', 'S')
                    && Point(p.x - 1, p.y) !in loop
                ) {
                    Point(p.x - 1, p.y)
                } else break@loop

            'L' -> loop += if (p.x + 1 in 0 until width && lines[p.y][p.x + 1] in setOf('-', 'J', '7', 'S')
                && Point(p.x + 1, p.y) !in loop
            ) {
                Point(p.x + 1, p.y)
            } else
            ...
            'J' -> loop += if (p.x - 1 in 0 until width && lines[p.y][p.x - 1] in setOf('-', 'L', 'F', 'S')
                && Point(p.x - 1, p.y) !in loop
            ) {
                Point(p.x - 1, p.y)
            } else
            ...
            '7' -> loop += if (p.x - 1 in 0 until width && lines[p.y][p.x - 1] in setOf('-', 'L', 'F', 'S')
                && Point(p.x - 1, p.y) !in loop
            ) {
                Point(p.x - 1, p.y)
            } else
                if (p.y + 1 in 0 until height && lines[p.y + 1][p.x] in setOf('|', 'L', 'J', 'S')
                    && Point(p.x, p.y + 1) !in loop
                ) {
                    Point(p.x, p.y + 1)
                } else break@loop

            'F' -> loop += if (p.x + 1 in 0 until width && lines[p.y][p.x + 1] in setOf('-', 'J', '7', 'S')
                && Point(p.x + 1, p.y) !in loop
            ) {
                Point(p.x + 1, p.y)
            } else
            ...
            'S' -> if (p.x - 1 in 0 until width && lines[p.y][p.x - 1] in setOf('-', 'L', 'F')
                && Point(p.x - 1, p.y) !in loop
            ) {
                loop += Point(p.x - 1, p.y)
            } else
                if (p.y - 1 in 0 until height && lines[p.y - 1][p.x] in setOf('|', 'F', '7')
                    && Point(p.x, p.y - 1) !in loop
                ) {
                    loop += Point(p.x, p.y - 1)
                } else
                    if (p.x + 1 in 0 until width && lines[p.y][p.x + 1] in setOf('-', 'J', '7')
                        && Point(p.x + 1, p.y) !in loop
                    ) {
                        loop += Point(p.x + 1, p.y)
                    } else
                        if (p.y + 1 in 0 until height && lines[p.y + 1][p.x] in setOf('|', 'L', 'J')
                            && Point(p.x, p.y + 1) !in loop
                        ) {
                            loop += Point(p.x, p.y + 1)
                        } else break@loop
    ...
    var edge = mutableSetOf<Point>()
    for (i in 1..loop.lastIndex) {
        if (loop[i].x == loop[i - 1].x && loop[i].y > loop[i - 1].y) {
            //down
            if (lines[loop[i - 1].y][loop[i - 1].x] == '7') {
                edge += Point(loop[i - 1].x, loop[i - 1].y - 1)
            }
            if (lines[loop[i].y][loop[i].x] == 'J') {
                edge += Point(loop[i].x, loop[i].y + 1)
            }
            edge += Point(loop[i - 1].x + 1, loop[i - 1].y)
            edge += Point(loop[i].x + 1, loop[i].y)
        } else if (loop[i].x == loop[i - 1].x && loop[i].y < loop[i - 1].y) {
            //up
            if (lines[loop[i].y][loop[i].x] == 'F') {
                edge += Point(loop[i].x, loop[i].y - 1)
            }
            if (lines[loop[i - 1].y][loop[i - 1].x] == 'L') {
                edge += Point(loop[i - 1].x, loop[i - 1].y + 1)
            }
            edge += Point(loop[i - 1].x - 1, loop[i - 1].y)
            edge += Point(loop[i].x - 1, loop[i].y)
            ...
            //right
            if (lines[loop[i - 1].y][loop[i - 1].x] == 'F') {
                edge += Point(loop[i - 1].x - 1, loop[i - 1].y)
            }
            if (lines[loop[i].y][loop[i].x] == '7') {
                edge += Point(loop[i].x + 1, loop[i].y)
            }
            edge += Point(loop[i - 1].x, loop[i - 1].y - 1)
            edge += Point(loop[i].x, loop[i].y - 1)
            ...
            //left
            if (lines[loop[i - 1].y][loop[i - 1].x] == 'J') {
                edge += Point(loop[i - 1].x + 1, loop[i - 1].y)
            }
            if (lines[loop[i - 1].y][loop[i - 1].x] == 'L') {
                edge += Point(loop[i].x - 1, loop[i].y)
    ...
    fun bfsFree(startPoint: Point): Set<Point> {
    ...
    var isAreaOutside = false
    for (x in 0 until width) if (Point(x, 0) in area) {
        isAreaOutside = true
        break
    }
    if (!isAreaOutside) for (y in 0 until height) if (Point(0, y) in area) {
        isAreaOutside = true
        break
    }
    if (!isAreaOutside) for (x in 0 until width) if (Point(x, height - 1) in area) {
        isAreaOutside = true
        break
    }
    if (!isAreaOutside) for (y in 0 until height) if (Point(width - 1, y) in area) {
        isAreaOutside = true
        break
    }
    ...
    for (y in 0 until height) {
        for (x in 0 until width) {
            if (Point(x, y) in loop) {
                print(lines[y][x])
            } else if (Point(x, y) in area) {
                print(if (isAreaOutside) 'O' else '*')
                if (!isAreaOutside) ++insideCount
            } else {
                print(if (isAreaOutside) '*' else 'O')
                if (isAreaOutside) ++insideCount
            }
        }
        println()
