...
public class AoC2018Day4Part2 {
    public static void main(String[] args) {
        ...
        assert result == 4455 : "unexpected result is " + result;
        ...
        assert result == 53024 : "unexpected result is " + result;
        System.out.println(result);
    }
    ...
    public static int test(String s) {
        ...
        for (Record record : records) {
            map.computeIfAbsent(record.id, key -> new int[60]);
        }

        Map<Integer, Integer> sleepTimeMap = new HashMap<>();
        for (int i = 0; i < records.size(); ++i) {
            Record startRecord = records.get(i);
            int selectedId = startRecord.id;
            int startMin = startRecord.min, endMin = 0;
            if (startRecord.event == Event.falls_asleep) {
                for (int j = i + 1; j < records.size(); ++j) {
                    Record endRecord = records.get(j);
                    if (endRecord.event == Event.wakes_up) {
                        // inside the one hour
                        endMin = endRecord.min;
                        break;
                    }
                }
            }
            int[] minutes = map.get(selectedId);
            for (int j = startMin; j < endMin; ++j) {
                ++minutes[j];
            }
        }

        int maxMinCount = 0, maxMin = 0, maxId = 0;
        for (Integer id : map.keySet()) {
            int[] minutes = map.get(id);
            for (int i = 0; i < minutes.length; ++i) {
                if (minutes[i] > maxMinCount) {
                    maxMinCount = minutes[i];
                    maxMin = i;
                    maxId = id;
                }
            }
